...
export default function WrappingScreen({ onSkip }: Props) {
  return (
    <div className="mx-auto flex min-h-[50vh] max-w-md flex-col items-center justify-center text-center animate-fade-in-up">
      ...
      {onSkip && (
        <button
          type="button"
          onClick={onSkip}
          className="mt-8 text-sm font-semibold text-brand underline-offset-2 hover:underline"
        >
          Taking too long? View results
        </button>
      )}
    </div>
  );
